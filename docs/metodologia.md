...
5. **Una medida solo cuenta si la respuesta es válida.**
   ...
   | Aislamiento de caché entre conversaciones (H-032) | Contrato **propio** otra vez, y por presencia, no por igualdad: lo que se mide es si la caché devuelve lo que guardó, no el formato. La respuesta normalizada debe contener el nonce de **su** conversación y **ninguno** de los ajenos. El nonce ajeno manda sobre todo lo demás —una respuesta con el propio *y* uno ajeno sigue siendo una fuga—, y se distingue de "no recupera": solo la contaminación descalifica `--cache-ram`. |
